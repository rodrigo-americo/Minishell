#!/bin/sh
# Gera docs/PROJECT_STANDARDS.md incluindo a lista de arquivos em src
set -e

# Caminho relativo (executar a partir da raiz do repositório)
ROOT="$(pwd)"
OUTDIR="$ROOT/docs"
OUTFILE="$OUTDIR/PROJECT_STANDARDS.md"

mkdir -p "$OUTDIR"

# lista de arquivos em src (ordenada)
FILE_LIST=$(find src -type f | sort)
...
- Expansões especiais:
  - \`$?\` → \`ft_itoa(shell->exit_status)\` (heap) — chamador libera.
  - \`$$\` → PID como string (obter via \`/proc/self\` ou \`shell->pid\`) — retorna heap.
...
- Incluir \`minishell.h\` é permitido para declarações globais, mas prefira includes diretos quando a dependência for específica.
...
# Apende a lista de arquivos encontrada em src
if [ -n "$FILE_LIST" ]; then
    printf "\n" >> "$OUTFILE"
    printf "Lista de arquivos encontrados em src/ (ordenado):\n\n" >> "$OUTFILE"
    printf "%s\n" "$FILE_LIST" | sed 's/^/  - /' >> "$OUTFILE"
else
