...
static char **process_wildcards_recursive(char **args, int i) {
  ...
  if (has_wildcard(args[i])) {
    ...
    if (expanded) {
      ...
      free_array(args);
      return (
          process_wildcards_recursive(result, i + 1)); // Continue from next arg
    }
  }
  return (process_wildcards_recursive(args, i + 1));
}
